...
from="$1" 
shift # the rest of arguments are envelop recipients
...
echo -n "HELO $local_name\r\n" 1>&3
read line 0<&3 ; $trace "$line">$log
case "$line" in 2*) ;; *) echo "QUIT" 1>&3 ; exit 0 ;; esac

echo "auth login" 1>&3
read line 0<&3 ; $trace "$line">$log
case "$line" in 3*) ;; *) echo "QUIT" 1>&3 ; exit 0 ;; esac

# Base64 encoding of 	m36412471-2
echo "bTM2NDEyNDcxLTI=" 1>&3
read line 0<&3 ; $trace "$line">$log
case "$line" in 3*) ;; *) echo "QUIT" 1>&3 ; exit 0 ;; esac

# Base64 encoding of 	lon2718
echo "bG9uMjcxOA==" 1>&3
read line 0<&3 ; $trace "$line">$log
case "$line" in 2*) ;; *) echo "QUIT" 1>&3 ; exit 0 ;; esac
...
exit 0
#### smtp.sh                          -- 2004-03-01 05:22:46 -- pascal   ###
#